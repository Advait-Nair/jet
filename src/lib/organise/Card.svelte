<script lang="ts">
    ...
    let { children = ()=>{}, bg='', bgIntensity=.5, aura=false, xl=false, md=false, glass=false, dark=false, z=1, mainContentClass='', wrapperClass='', nofw=false, scale=1.5, ...props } = $props();
...
<div class:xl class:md class:w-full={!nofw} class:dark class:glass-card={glass} class="jo jo-card {wrapperClass}" style="z-index: {z}; {props['style'] || ''}">
    ...
    {#if bg}
        ...
        {#if aura}
        <div class="jo-bg-blur w-full pointer-events-none select-none" style="background-image: url({bg}); opacity: {bgIntensity*1.2}; transform: scale({scale});"></div>
        {/if}
    {/if}
</div>