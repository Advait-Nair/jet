<script lang="ts">
    ...
    warn(id);
    let h3:any = $state(null);
</script>

<h3 bind:this={h3} class:jtt-hash-touch={!nohashlink} id={procHash(id || generateId('heading', h3))} class="jtt jtt-heading {props['class'] || ''}" {...removeClass(props)}>
    {@render children?.()}
</h3>
